...
{
    public partial class FormManageUserEmployee : Form
    {
        ...
        public FormManageUserEmployee(string username)
        {
            _username = username;
        ...
        private void btnUpdateAll_Click(object sender, EventArgs e)
        {
            ...
            if (currentPassword == txtCurrentPassword.Text)
            {
                ...
                {
                    MessageBox.Show(@"New password required", @"Required New Password", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    return;
                }
                using (SqlConnection conn = new SqlConnection(Connectionstring))
                {
            ...
            using (SqlConnection conn = new SqlConnection(Connectionstring))
            {
                string query = "UPDATE [User] SET email = '" + ((txtEmail.Text == "") ? _user.Email : txtEmail.Text) +
                               "' WHERE username = @nam";
                using (SqlCommand cmd = new SqlCommand(query, conn))
                {
                    conn.Open();
        ...
        {
            if (txtCurrentPassword.Text == "" && txtEmail.Text == "")
            {
